...
for d_idx, depth in enumerate(distance):
    ...
    while amp < max_current and not is_spike:
        ...
        print("running cell {2} soma distance from electrode:\
         {0} current intensity: {1}".format(z_cell_pos[cell_id], amp, names[cell_id]))
...
if cell_id == 0:
    ...
    axview = plt.subplot(131, title="2D view XZ", aspect='auto', xlabel="x [$\mu$m]", ylabel="z [$\mu$m]")
    for nc in range(0, n_cells):
        [axview.plot([cells[nc]['xstart'][idx], cells[nc]['xend'][idx]], [cells[nc]['zstart'][idx],
                     cells[nc]['zend'][idx]], '-',
                     c=col[nc], clip_on=False) for idx in range(cells[nc]['totnsegs'])]
        [axview.scatter(cells[nc]['xmid'][ap], cells[nc]['zmid'][ap],
                        c='k', marker='*') for ap in gather_current[nc]['ap_loc']]
        axview.text(cells[nc]['xmid'][0], cells[nc]['zmid'][0], names[cells[nc]['rank']])
    ...
    axview.scatter(source_xs, source_zs, c=source_amps)
    axview2 = plt.subplot(132, title="2D view YZ", aspect='auto', xlabel="y [$\mu$m]", ylabel="z [$\mu$m]")
    for nc in range(0, n_cells):
        [axview2.plot([cells[nc]['ystart'][idx], cells[nc]['yend'][idx]], [cells[nc]['zstart'][idx],
                      cells[nc]['zend'][idx]], '-',
                      c=col[nc], clip_on=False) for idx in range(cells[nc]['totnsegs'])]
        [axview2.scatter(cells[nc]['ymid'][ap], cells[nc]['zmid'][ap],
                         c='k', marker='*') for ap in gather_current[nc]['ap_loc']]
        axview2.text(cells[nc]['ymid'][0], cells[nc]['zmid'][0], names[cells[nc]['rank']])
    ...
    axview2.scatter(source_ys, source_zs, c=source_amps)
    axview3 = plt.subplot(133, title="2D view XY", aspect='auto', xlabel="x [$\mu$m]", ylabel="y [$\mu$m]")
    for nc in range(0, n_cells):
        [axview3.plot([cells[nc]['xstart'][idx], cells[nc]['xend'][idx]], [cells[nc]['ystart'][idx],
                      cells[nc]['yend'][idx]], '-',
                      c=col[nc], clip_on=False) for idx in range(cells[nc]['totnsegs'])]
        [axview3.scatter(cells[nc]['xmid'][ap], cells[nc]['ymid'][ap],
                         c='k', marker='*') for ap in gather_current[nc]['ap_loc']]
        axview3.text(cells[nc]['xmid'][0], cells[nc]['ymid'][0], names[cells[nc]['rank']])
    ...
    axview3.scatter(source_xs, source_ys, c=source_amps)
    plt.tight_layout()
    plt.savefig('outputs/fram/3figs.png', dpi=200)
    ...
    # cb.set_label('mV', labelpad=-10)
    if max_current > 0:
        plt.savefig("outputs/fram/geometry_selectivity_L5_" + name_shape_ecog + "_positive.png", dpi=300)
    else:
        plt.savefig("outputs/fram/geometry_selectivity_L5_" + name_shape_ecog + "_negative.png", dpi=300)
    ...
    plt.legend(loc="upper left")
    if max_current > 0:
        plt.savefig("outputs/fram/selectivity_L5_" + name_shape_ecog + "_positive.png", dpi=300)
    else:
        plt.savefig("outputs/fram/selectivity_L5_" + name_shape_ecog + "_negative.png", dpi=300)
